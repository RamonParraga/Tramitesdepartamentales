<div class="row" id="adm">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        ...
        <h2><i class="fa fa-edit"></i> Registrar Sector</h2>
          <div class="clearfix"></div>
        </div>
        <div class="x_content">
          <br>
        <form  method="POST"  id="id_frmsector"  data-parsley-validate="" class="form-horizontal form-label-left" novalidate="" accept-charset="UTF-8">
            {{ csrf_field() }}
          <input id="method_sector" type="hidden" name="_method" value="POST">
           ...
           {{-- <div class="form-group departamento">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12"> 
                <div class="chosen-select-conten">
                    <select data-placeholder="Seleccione una bodega" name="idbodega" id="idbodega" class="chosen-select form-control" tabindex="5">  
                                @if(isset($listaBodega))
                                @foreach($listaBodega as $bodega)
                                <optgroup label="{{$bodega->nombre}}">
                                @foreach($bodega->departamento as $departamento)
                                <option class="opcion_gestion" value="{{$departamento->iddepartamento}}">{{$departamento->nombre}}</option>
                                @endforeach
                                  <optgroup>
                                @endforeach
                                @endif   
                ...
                <div class="chosen-select-conten">
                    <select data-placeholder="Seleccione una bodega" name="idbodega" id="idbodega" class="chosen-select form-control" tabindex="5">  
                                    @if(isset($listaBodega))
                                        @foreach($listaBodega as $bodega) 
                                          <option value=""></option>
                                            <option class="option_bodega" value="{{$bodega->id_bodega}}">{{$bodega->nombre}}</option>
                                        @endforeach
                                    @endif
                                </select>                    
                </div>
         ...
         {{--  <div id="content_bodegas" class="form-group hidden" style="margin-bottom: 0px;">
            ...
            <div class="form-group">
              <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                ...
                <button type="button" id="btn_sectorcancelar" class="btn btn-warning hidden">Cancelar</button>
              </div>
            </div>
          </form>
        </div>
