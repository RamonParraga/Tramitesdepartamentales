...
OrgChart.prototype._draw = function(t, m, O, v) {
    ...
    this.server.read(t, this.width(), this.height(), e, m, O, function(t) {
        m != OrgChart.action.exporting && (y.nodes = t.nodes),
        ...
        var i = y.getViewBox()
          , a = t.viewBox;
        for (var n in y._initialized || (y.element.innerHTML = '<div id="bg-tests"></div>'),
        t.visibleNodes) {
            var o = t.visibleNodes[n];
            e += y.ui.node(o, y._get(n), t.animations, y.config, void 0, void 0, void 0, m, r, y),
            e += y.ui.secondLinks(t.visibleNodes, o, y.config, t.boundary, r),
            e += y.ui.link(o, y, y.config, r),
            e += y.ui.expandCollapse(o, y.config, m, r)
        }
        if (e += y.ui.lonely(y.config),
        ...
        m !== OrgChart.action.exporting) {
            ...
            if (y._initialized) {
                ...
                y.element.innerHTML = y.ui.css() + y.ui.exportMenuButton(y.config) + C,
                y._attachInitEventHandlers(),
                y._attachEventHandlers(),
                y.config.showXScroll === OrgChart.scroll.visible && (y.xScrollUI.create(y.width(), y.config.padding),
                y.xScrollUI.setPosition(),
                y.xScrollUI.addListener(y.getSvg())),
                y.config.showYScroll === OrgChart.scroll.visible && (y.yScrollUI.create(y.height(), y.config.padding),
                y.yScrollUI.setPosition(),
                y.yScrollUI.addListener(y.getSvg())),
            ...
            m === OrgChart.action.centerNode ? OrgChart.anim(y.getSvg(), {
                ...
                y._showAfterAnimation(),
                d = !0)
            }) : !i || !y.response || i[0] == y.response.viewBox[0] && i[1] == y.response.viewBox[1] && i[2] == y.response.viewBox[2] && i[3] == y.response.viewBox[3] || m !== OrgChart.action.insert && m !== OrgChart.action.expandCollapse && m !== OrgChart.action.update && m !== OrgChart.action.init ? 0 == c[0].length && (d || (v && v(),
            OrgChart.events.publish("redraw", [y]),
            d = !0)) : OrgChart.anim(y.getSvg(), {
                viewbox: i
            }, {
                viewbox: y.response.viewBox
...
OrgChart.server._createNodes = function(t, e, r, i) {
            ...
            if (b) {
                if (b.parent)
                    for (var w = (_ = b.parent).children.length - 1; 0 <= w; w--) {
                        (F = _.children[w]) == b && (F.parent = null,
    ...
    for (c = 0; c < s.length; c++) {
        ...
        for (w = _.children.length - 1; 0 <= w; w--) {
            (F = _.children[w]).isAssistant ? (F.parent = null,
            _.children.splice(w, 1),
            _.childrenIds.splice(w, 1),
            A.splice(0, 0, F.id)) : (F.pid = k.id,
            (F.parent = k).children.push(F),
            k.childrenIds.push(F.id),
            _.children.splice(w, 1))
        }
        if (A.length % 2) {
            ...
            (W = new OrgChart.node(_.id + "_transperant",S.pid,null,[],I)).isAssistant = !0,
            n[W.id] = W,
        ...
        for (w = A.length - 1; 0 <= w; w--) {
            if (w % 2 && w != A.length - 1)
                (J = new OrgChart.node(_.id + "_split_" + E,null,null,[],"split")).isAssistant = !0,
                n[J.id] = J,
                A.splice(w, 0, J.id),
                E++;
            else
        ...
        for (w = 0; w < A.length; w += 3) {
            ...
            for (var P = M; "split" == P.templateName && (P = P.parent); )
                P.childrenIds.push(L.id),
                P.childrenIds.push(B.id),
    ...
    if (e == OrgChart.action.centerNode) {
        var H = n[r.id];
        H || (H = n[h[r.id]],
        OrgChart.server._setNodeGroupState(H, OrgChart.EXPAND, t.orientation)),
        H.state = OrgChart.EXPAND,
        -1 != (D = O.indexOf(H.id)) && O.splice(D, 1);
        ...
        if (r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
            for (c = 0; c < H.childrenIds.length; c++) {
                if (-1 != (D = O.indexOf(H.childrenIds[c])) && O.splice(D, 1),
                (F = n[H.childrenIds[c]]).state = OrgChart.EXPAND,
                !F.isAssistant && !OrgChart.server._isSplit(F))
                    for (w = 0; w < F.childrenIds.length; w++) {
                        var D, G = n[F.childrenIds[w]];
                        -1 == (D = O.indexOf(F.childrenIds[w])) && O.push(F.childrenIds[w]),
            ...
            for (w = _.children.length - 1; 0 <= w; w--) {
                if (_.children[w] == y)
                    ...
                    "split" == y.templateName || "split" == _.templateName) {
                        if (M = y.parent) {
                            for (; "split" == M.templateName; )
                                M = M.parent;
                            for (var R = 0; R < M.childrenIds.length; R++) {
                                var X = M.childrenIds[R];
                                -1 == M.collapsedChildrenIds.indexOf(X) && M.collapsedChildrenIds.push(X)
                            }
                        }
                    } else
                        y.parent = null
    ...
    if (t.layout == OrgChart.mixed)
        for (c = 0; c < o.length; c++) {
            if (0 == (y = n[o[c]]).childrenIds.length && !y.isAssistant && "split" != y.templateName)
                if (_ = y.parent)
                    for (w = _.children.length - 1; 0 <= w; w--) {
                        if ((F = _.children[w]) != y && 0 == F.childrenIds.length && !F.isAssistant && "split" != F.templateName) {
                            F.parent = y,
                            F.pid = y.id,
                            F.layout = OrgChart.mixed,
                            _.children.splice(w, 1);
                            var q = y.children.length;
                            y.children[q] = F,
    ...
    if (t.layout == OrgChart.tree || t.layout == OrgChart.treeRightOffset || t.layout == OrgChart.treeLeftOffset) {
        ...
        for (c = 0; c < o.length; c++) {
            if (0 == (y = n[o[c]]).childrenIds.length && !y.isAssistant && "split" != y.templateName)
                (_ = y.parent) && -1 == Y.indexOf(_.id) && Y.push(_.id)
        }
        for (c = 0; c < Y.length; c++) {
            ...
            var V = [];
            for (w = _.children.length - 1; 0 <= w; w--) {
                if (0 == (F = _.children[w]).childrenIds.length && !F.isAssistant && "split" != F.templateName && (F.parent && "split" != F.parent.templateName || F.parent.isAssistant)) {
                    if ((F = _.children[w]).parent = null,
                    F.layout = OrgChart.tree,
                    _.children.splice(w, 1),
                    _.childrenIds.splice(w, 1),
                    t.layout == OrgChart.treeRightOffset && V.splice(0, 0, F.id),
                    t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
                        ...
                        (W = new OrgChart.node(F.id + "_transperant",null,null,[],I)).layout = OrgChart.tree,
                        n[W.id] = W,
                        V.splice(0, 0, W.id)
                    }
                    t.layout != OrgChart.treeRightOffset && V.splice(0, 0, F.id)
            ...
            w = V.length - 1; 0 <= w; w--) {
                var J;
                if (w % 2 && w != V.length - 1)
                    (J = new OrgChart.node(_.id + "_split_" + E,null,null,[],"split")).layout = OrgChart.tree,
                    n[J.id] = J,
                    V.splice(w, 0, J.id),
                    E++;
                else
    ...
    if (r != OrgChart.action.init && r != OrgChart.action.exporting || !t) {
        ...
        if (t.tags)
            for (var l = 0; l < t.tags.length; l++) {
                var s = t.tags[l];
                i.tags && i.tags[s] && null != i.tags[s].state && (i.tags[s].state == OrgChart.COLLAPSE && n.push(t.id),
                t.state = i.tags[s].state)
            }
        if (i.collapse.level) {
            var h = 0;
            if (!i.roots || -1 == i.roots.indexOf(t.id)) {
                for (var d = e[t.pid]; OrgChart.server._isSplit(d); )
                    d = e[d.pid];
            ...
            if (h >= i.collapse.level) {
                ...
                t.state = OrgChart.COLLAPSE) : c || i.collapse.allChildren || h != i.collapse.level || (n.push(t.id),
    ...
    for (var S in g) {
        ...
        if (OrgChart.server._isVisible(N, i, s, d)) {
            ...
            if (d == OrgChart.action.expandCollapse && f && f[N.id]) {
                ...
                var L = g[c.id];
                if (L) {
                    for (E = N.parent; null != E && -1 == c.ids.indexOf(N.id) && -1 == c.ids.indexOf(E.id); )
                        E = E.parent;
                    E && (I = {
                        x: L.x + L.w / 2 - N.w / 2,
                        y: L.y + L.h / 2 - N.h / 2
            ...
            } else if (d == OrgChart.action.insert && c && c.insertedNodeId == N.id)
                I = {
                    x: N.parent.x,
                    y: N.parent.y
                };
                ...
                d !== OrgChart.action.exporting && r && !r[N.id] && (m[0].push(N.id),
                m[1].push({
                    opacity: 0
                }),
                m[2].push({
            ...
            null != I && null != I.x && null != I.y && (I.x == N.x && I.y == N.y || (m[0].push(N.id),
            m[1].push({
